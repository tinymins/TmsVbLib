Attribute VB_Name = "ModTmsHttpHelper"
Option Explicit
...
Function ExtractUrl(ByVal strUrl As String) As URL
    ...
    If intPos1 > 0 Then
        retURL.Scheme = Mid(strUrl, InStr(strUrl, " ") + 1, intPos1 - InStr(strUrl, " ") - 1)
    ...
    intPos1 = InStr(strUrl, "?")
    intPos2 = InStr(intPos1 + 1, strUrl, " ")
    
    If intPos1 > 0 Then
        retURL.URI = Mid(strUrl, 1, intPos1 - 1)
        retURL.Query = Mid(strUrl, intPos1 + 1, intPos2 - intPos1 - 1)
    Else
        retURL.URI = strUrl
    End If
